...
void cheat::zombie_bone() {
    ...
    while (!p_entity_iterator->IsEnd())
    {
        ...
        if (strstr(get_name, _("grunt_base")))
        {
            const float* b_color = menu::zombie_color;
            ...
            {
	            const ImVec2 calc_size = ImGui::CalcTextSize(_("Zombie"), nullptr, false);
                ImGui::GetOverlayDrawList()->AddText(ImVec2(screen.x - calc_size.x / 2 - 1, screen.y), ImColor{ b_color[0], b_color[1], b_color[2], b_color[3] }, _("Zombie"));
