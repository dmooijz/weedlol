...
void cheat::immolator() {
    ...
    while (!p_entity_iterator->IsEnd())
    {
        ...
        if (strstr(get_name, _("Immolator")))
        {
            const float* b_color = menu::immolator_color;

            skeleton::draw_skeleton(p_entity, { b_color[0], b_color[1], b_color[2], b_color[3] });
            if (auto root_bone = deps::get_bone_name(p_entity, _("root")); deps::world_to_screen(root_bone, screen))
            {
                const ImVec2 calc_size = ImGui::CalcTextSize(_("Immolator"), nullptr, false);
                ImGui::GetOverlayDrawList()->AddText(ImVec2(screen.x - calc_size.x / 2 - 1, screen.y), ImColor{ b_color[0], b_color[1], b_color[2], b_color[3] }, _("Immolator"));
