...
ImGuiIO::ImGuiIO()
{
	...
	IniSavingRate = 5.0f;
	IniFilename = _("imgui.ini");
	LogFilename = _("imgui_log.txt");
...
{
	return GImGui->IO.GetClipboardTextFn ? GImGui->IO.GetClipboardTextFn(GImGui->IO.ClipboardUserData) : _("");
}
...
void ImGui::NewFrame()
{
	...
	IM_ASSERT(g.IO.DeltaTime >= 0.0f && _("Need a positive DeltaTime (zero is tolerated but will cause some timing issues)"));
	IM_ASSERT(g.IO.DisplaySize.x >= 0.0f && g.IO.DisplaySize.y >= 0.0f && _("Invalid DisplaySize value"));
	IM_ASSERT(g.IO.Fonts->Fonts.Size > 0 && _("Font Atlas not created. Did you call io.Fonts->GetTexDataAsRGBA32 / GetTexDataAsAlpha8 ?"));
	IM_ASSERT(g.IO.Fonts->Fonts[0]->IsLoaded() && _("Font Atlas not created. Did you call io.Fonts->GetTexDataAsRGBA32 / GetTexDataAsAlpha8 ?"));
...
static void SettingsHandlerWindow_ReadLine(ImGuiContext*, ImGuiSettingsHandler*, void* entry, const char* line)
{
	...
	float x, y;
	int i;
	if (sscanf(line, _("Pos=%f,%f"), &x, &y) == 2)         settings->Pos = ImVec2(x, y);
	else if (sscanf(line, _("Size=%f,%f"), &x, &y) == 2)   settings->Size = ImMax(ImVec2(x, y), GImGui->Style.WindowMinSize);
	else if (sscanf(line, _("Collapsed=%d"), &i) == 1)     settings->Collapsed = (i != 0);
}
...
void ImGui::Initialize()
{
	...
	ini_handler.TypeHash = ImHash(_("Window"), 0, 0);
...
static void LoadIniSettingsFromDisk(const char* ini_filename)
{
	if (!ini_filename)
		return;
	char* file_data = (char*)ImFileLoadToMemory(ini_filename, _("rb"), NULL, +1);
	if (!file_data)
		return;
...
static void SaveIniSettingsToDisk(const char* ini_filename)
{
	...
	FILE* f = ImFileOpen(ini_filename, _("wt"));
	if (!f)
		return;
...
bool ImGui::BeginPopupEx(ImGuiID id, ImGuiWindowFlags extra_flags)
{
	...
	if (extra_flags & ImGuiWindowFlags_ChildMenu)
		ImFormatString(name, IM_ARRAYSIZE(name), _("##Menu_%02d"), g.CurrentPopupStack.Size);    // Recycle windows based on depth
	else
...
static void CheckStacksSize(ImGuiWindow* window, bool write)
{
	...
	{ int current = window->IDStack.Size;       if (write) *p_backup = current; else IM_ASSERT(*p_backup == current && _("PushID/PopID or TreeNode/TreePop Mismatch!"));   p_backup++; }    // Too few or too many PopID()/TreePop()
	{ int current = window->DC.GroupStack.Size; if (write) *p_backup = current; else IM_ASSERT(*p_backup == current && _("BeginGroup/EndGroup Mismatch!"));                p_backup++; }    // Too few or too many EndGroup()
	{ int current = g.CurrentPopupStack.Size;   if (write) *p_backup = current; else IM_ASSERT(*p_backup == current && _("BeginMenu/EndMenu or BeginPopup/EndPopup Mismatch")); p_backup++; }// Too few or too many EndMenu()/EndPopup()
	{ int current = g.ColorModifiers.Size;      if (write) *p_backup = current; else IM_ASSERT(*p_backup == current && _("PushStyleColor/PopStyleColor Mismatch!"));       p_backup++; }    // Too few or too many PopStyleColor()
	{ int current = g.StyleModifiers.Size;      if (write) *p_backup = current; else IM_ASSERT(*p_backup == current && _("PushStyleVar/PopStyleVar Mismatch!"));           p_backup++; }    // Too few or too many PopStyleVar()
	{ int current = g.FontStack.Size;           if (write) *p_backup = current; else IM_ASSERT(*p_backup == current && _("PushFont/PopFont Mismatch!"));                   p_backup++; }    // Too few or too many PopFont()
	...
	if (first_begin_of_the_frame)
	{
		...
		if (!(flags & ImGuiWindowFlags_NoTitleBar))
		{
			...
			if (p_open != NULL)
			{
				...
				if (CloseButton(window->GetID(_("#CLOSE")), window->Rect().GetTR() + ImVec2(-PAD - rad, PAD + rad), rad))
					*p_open = false;
			}
...
void ImGui::Scrollbar(ImGuiLayoutType direction)
{
	...
	const ImGuiStyle& style = g.Style;
	const ImGuiID id = window->GetID(horizontal ? _("#SCROLLX") : _("#SCROLLY"));
...
const char* ImGui::GetStyleColorName(ImGuiCol idx)
{
	...
	case ImGuiCol_CloseButton: return _("CloseButton");
	case ImGuiCol_CloseButtonHovered: return _("CloseButtonHovered");
	case ImGuiCol_CloseButtonActive: return _("CloseButtonActive");
	...
	case ImGuiCol_ModalWindowDarkening: return _("ModalWindowDarkening");
...
void ImGui::LogToFile(int max_depth, const char* filename)
{
	...
	g.LogFile = ImFileOpen(filename, _("ab"));
	if (!g.LogFile)
	{
...
void ImGui::LogButtons()
{
	...
	PushID(_("LogButtons"));
	const bool log_to_tty = Button(_("Log To TTY")); SameLine();
	const bool log_to_file = Button(_("Log To File")); SameLine();
	const bool log_to_clipboard = Button(_("Log To Clipboard")); SameLine();
	PushItemWidth(80.0f);
	PushAllowKeyboardFocus(false);
	SliderInt(_("Depth"), &g.LogAutoExpandMaxDepth, 0, 9, NULL);
...
static inline void DataTypeFormatString(ImGuiDataType data_type, void* data_ptr, int decimal_precision, char* buf, int buf_size)
{
	if (data_type == ImGuiDataType_Int)
	{
		if (decimal_precision < 0)
			ImFormatString(buf, buf_size, _("%d"), *(int*)data_ptr);
		else
			ImFormatString(buf, buf_size, _("%.*d"), decimal_precision, *(int*)data_ptr);
	}
	else if (data_type == ImGuiDataType_Float)
	{
		if (decimal_precision < 0)
			ImFormatString(buf, buf_size, _("%f"), *(float*)data_ptr);     // Ideally we'd have a minimum decimal precision of 1 to visually denote that it is a float, while hiding non-significant digits?
		else
			ImFormatString(buf, buf_size, _("%.*f"), decimal_precision, *(float*)data_ptr);
	}
}
...
static bool DataTypeApplyOpFromText(const char* buf, const char* initial_value_buf, ImGuiDataType data_type, void* data_ptr, const char* scalar_format)
{
	...
	if (data_type == ImGuiDataType_Int)
	{
		...
		float arg1f = 0.0f;
		if (op == '+') { if (sscanf(buf, _("%f"), &arg1f) == 1) *v = (int)(arg0i + arg1f); }               
		else if (op == '*') { if (sscanf(buf, _("%f"), &arg1f) == 1) *v = (int)(arg0i * arg1f); }                 // Multiply
		else if (op == '/') { if (sscanf(buf, _("%f"), &arg1f) == 1 && arg1f != 0.0f) *v = (int)(arg0i / arg1f); }// Divide
...
bool ImGui::SliderAngle(const char* label, float* v_rad, float v_degrees_min, float v_degrees_max)
{
	...
	bool value_changed = SliderFloat(label, &v_deg, v_degrees_min, v_degrees_max, _("%.0f deg"), 1.0f);
...
bool ImGui::SliderFloatN(const char* label, float* v, int components, float v_min, float v_max, const char* display_format, float power)
{
	...
	{
		PushID(i);
		value_changed |= SliderFloat(_("##v"), &v[i], v_min, v_max, display_format, power);
...
bool ImGui::SliderIntN(const char* label, int* v, int components, int v_min, int v_max, const char* display_format)
{
	...
	{
		PushID(i);
		value_changed |= SliderInt(_("##v"), &v[i], v_min, v_max, display_format);
		SameLine(0, g.Style.ItemInnerSpacing.x);
		PopID();
		PopItemWidth();
	}
...
bool ImGui::DragFloatN(const char* label, float* v, int components, float v_speed, float v_min, float v_max, const char* display_format, float power)
{
	...
	{
		PushID(i);
		value_changed |= DragFloat(_("##v"), &v[i], v_speed, v_min, v_max, display_format, power);
		SameLine(0, g.Style.ItemInnerSpacing.x);
		PopID();
		PopItemWidth();
	}
...
bool ImGui::DragFloatRange2(const char* label, float* v_current_min, float* v_current_max, float v_speed, float v_min, float v_max, const char* display_format, const char* display_format_max, float power)
{
	...
	bool value_changed = DragFloat(_("##min"), v_current_min, v_speed, (v_min >= v_max) ? -FLT_MAX : v_min, (v_min >= v_max) ? *v_current_max : ImMin(v_max, *v_current_max), display_format, power);
	PopItemWidth();
	SameLine(0, g.Style.ItemInnerSpacing.x);
	value_changed |= DragFloat(_("##max"), v_current_max, v_speed, (v_min >= v_max) ? *v_current_min : ImMax(v_min, *v_current_min), (v_min >= v_max) ? FLT_MAX : v_max, display_format_max ? display_format_max : display_format, power);
	PopItemWidth();
	SameLine(0, g.Style.ItemInnerSpacing.x);
...
bool ImGui::DragIntN(const char* label, int* v, int components, float v_speed, int v_min, int v_max, const char* display_format)
{
	...
	{
		PushID(i);
		value_changed |= DragInt(_("##v"), &v[i], v_speed, v_min, v_max, display_format);
		SameLine(0, g.Style.ItemInnerSpacing.x);
		PopID();
		PopItemWidth();
	}
...
bool ImGui::DragIntRange2(const char* label, int* v_current_min, int* v_current_max, float v_speed, int v_min, int v_max, const char* display_format, const char* display_format_max)
{
	...
	bool value_changed = DragInt(_("##min"), v_current_min, v_speed, (v_min >= v_max) ? INT_MIN : v_min, (v_min >= v_max) ? *v_current_max : ImMin(v_max, *v_current_max), display_format);
	PopItemWidth();
	SameLine(0, g.Style.ItemInnerSpacing.x);
	value_changed |= DragInt(_("##max"), v_current_max, v_speed, (v_min >= v_max) ? *v_current_min : ImMax(v_min, *v_current_min), (v_min >= v_max) ? INT_MAX : v_max, display_format_max ? display_format_max : display_format);
	PopItemWidth();
	SameLine(0, g.Style.ItemInnerSpacing.x);
...
bool ImGui::Checkbox(const char* label, bool* v)
{
	...
	if (g.LogEnabled)
		LogRenderedText(&text_bb.Min, *v ? _("[x]") : _("[ ]"));
	if (label_size.x > 0.0f)
...
bool ImGui::RadioButton(const char* label, bool active)
{
	...
	if (g.LogEnabled)
		LogRenderedText(&text_bb.Min, active ? _("(x)") : _("( )"));
	if (label_size.x > 0.0f)
		RenderText(text_bb.Min, label);
...
bool ImGui::InputTextEx(const char* label, char* buf, int buf_size, const ImVec2& size_arg, ImGuiInputTextFlags flags, ImGuiTextEditCallback callback, void* user_data)
{
	...
	const bool user_scrolled = is_multiline && g.ActiveId == 0 && edit_state.Id == id && g.ActiveIdPreviousFrame == draw_window->GetIDNoKeepAlive(_("#SCROLLY"));
	...
	const bool is_currently_scrolling = (edit_state.Id == id && is_multiline && g.ActiveId == draw_window->GetIDNoKeepAlive(_("#SCROLLY")));
...
bool ImGui::InputScalarEx(const char* label, ImGuiDataType data_type, void* data_ptr, void* step_ptr, void* step_fast_ptr, const char* scalar_format, ImGuiInputTextFlags extra_flags)
{
	...
	if (InputText(_(""), buf, IM_ARRAYSIZE(buf), extra_flags))
	...
	if (step_ptr)
	{
		PopItemWidth();
		SameLine(0, style.ItemInnerSpacing.x);
		if (ButtonEx(_("-"), button_sz, ImGuiButtonFlags_Repeat | ImGuiButtonFlags_DontClosePopups))
		{
...
bool ImGui::InputFloat(const char* label, float* v, float step, float step_fast, int decimal_precision, ImGuiInputTextFlags extra_flags)
{
	...
	if (decimal_precision < 0)
		strcpy(display_format, _("%f"));      // Ideally we'd have a minimum decimal precision of 1 to visually denote that this is a float, while hiding non-significant digits? %f doesn't have a minimum of 1
	else
		ImFormatString(display_format, IM_ARRAYSIZE(display_format), _("%%.%df"), decimal_precision);
...
bool ImGui::InputInt(const char* label, int* v, int step, int step_fast, ImGuiInputTextFlags extra_flags)
{
	...
	const char* scalar_format = (extra_flags & ImGuiInputTextFlags_CharsHexadecimal) ? _("%08X") : _("%d");
...
bool ImGui::InputFloatN(const char* label, float* v, int components, int decimal_precision, ImGuiInputTextFlags extra_flags)
{
	...
	{
		PushID(i);
		value_changed |= InputFloat(_("##v"), &v[i], 0, 0, decimal_precision, extra_flags);
		SameLine(0, g.Style.ItemInnerSpacing.x);
		PopID();
		PopItemWidth();
	}
...
bool ImGui::InputIntN(const char* label, int* v, int components, ImGuiInputTextFlags extra_flags)
{
	...
	{
		PushID(i);
		value_changed |= InputInt(_("##v"), &v[i], 0, 0, extra_flags);
		SameLine(0, g.Style.ItemInnerSpacing.x);
		PopID();
		PopItemWidth();
	}
...
bool ImGui::BeginCombo(const char* label, const char* preview_value, ImGuiComboFlags flags)
{
	...
	char name[16];
	ImFormatString(name, IM_ARRAYSIZE(name), _("##Combo_%02d"), g.CurrentPopupStack.Size); // Recycle windows based on depth
...
bool ImGui::BeginMainMenuBar()
{
	...
	if (!Begin(_("##MainMenuBar"), NULL, ImGuiWindowFlags_NoTitleBar | ImGuiWindowFlags_NoResize | ImGuiWindowFlags_NoMove | ImGuiWindowFlags_NoScrollbar | ImGuiWindowFlags_NoSavedSettings | ImGuiWindowFlags_MenuBar)
		|| !BeginMenuBar())
	{
		End();
		PopStyleVar(2);
...
void ImGui::ColorTooltip(const char* text, const float* col, ImGuiColorEditFlags flags)
{
	...
	ColorButton(_("##preview"), ImVec4(col[0], col[1], col[2], col[3]), (flags & (ImGuiColorEditFlags_NoAlpha | ImGuiColorEditFlags_AlphaPreview | ImGuiColorEditFlags_AlphaPreviewHalf)) | ImGuiColorEditFlags_NoTooltip, sz);
	SameLine();
...
void ImGui::ColorEditOptionsPopup(const float* col, ImGuiColorEditFlags flags)
{
	...
	if (allow_opt_inputs)
	{
		if (RadioButton(_("RGB"), (opts & ImGuiColorEditFlags_RGB) ? 1 : 0)) opts = (opts & ~ImGuiColorEditFlags__InputsMask) | ImGuiColorEditFlags_RGB;
		if (RadioButton(_("HSV"), (opts & ImGuiColorEditFlags_HSV) ? 1 : 0)) opts = (opts & ~ImGuiColorEditFlags__InputsMask) | ImGuiColorEditFlags_HSV;
		if (RadioButton(_("HEX"), (opts & ImGuiColorEditFlags_HEX) ? 1 : 0)) opts = (opts & ~ImGuiColorEditFlags__InputsMask) | ImGuiColorEditFlags_HEX;
	}
	if (allow_opt_datatype)
	{
		if (allow_opt_inputs) Separator();
		if (RadioButton(_("0..255"), (opts & ImGuiColorEditFlags_Uint8) ? 1 : 0)) opts = (opts & ~ImGuiColorEditFlags__DataTypeMask) | ImGuiColorEditFlags_Uint8;
		if (RadioButton(_("0.00..1.00"), (opts & ImGuiColorEditFlags_Float) ? 1 : 0)) opts = (opts & ~ImGuiColorEditFlags__DataTypeMask) | ImGuiColorEditFlags_Float;
	}
...
static void ColorPickerOptionsPopup(ImGuiColorEditFlags flags, const float* ref_col)
{
	...
	if ((!allow_opt_picker && !allow_opt_alpha_bar) || !ImGui::BeginPopup(_("context")))
		return;
	ImGuiContext& g = *GImGui;
	if (allow_opt_picker)
	{
		...
		for (int picker_type = 0; picker_type < 2; picker_type++)
		{
			...
			if (ImGui::Selectable(_("##selectable"), false, 0, picker_size)) // By default, Selectable() is closing popup
				g.ColorEditOptions = (g.ColorEditOptions & ~ImGuiColorEditFlags__PickerMask) | (picker_flags & ImGuiColorEditFlags__PickerMask);
			ImGui::SetCursorScreenPos(backup_pos);
			ImVec4 dummy_ref_col;
			memcpy(&dummy_ref_col.x, ref_col, sizeof(float) * (picker_flags & ImGuiColorEditFlags_NoAlpha ? 3 : 4));
			ImGui::ColorPicker4(_("##dummypicker"), &dummy_ref_col.x, picker_flags);
			ImGui::PopID();
		}
		ImGui::PopItemWidth();
	}
	if (allow_opt_alpha_bar)
	{
		...
		ImGui::CheckboxFlags(_("Alpha Bar"), (unsigned int*)&g.ColorEditOptions, ImGuiColorEditFlags_AlphaBar);
	}
	ImGui::EndPopup();
}
...
bool ImGui::ColorEdit4(const char* label, float col[4], ImGuiColorEditFlags flags)
{
	...
	if ((flags & (ImGuiColorEditFlags_RGB | ImGuiColorEditFlags_HSV)) != 0 && (flags & ImGuiColorEditFlags_NoInputs) == 0)
	{
		...
		const char* ids[4] = { _("##X"), _("##Y"), _("##Z"), _("##W") };
		const char* fmt_table_int[3][4] =
		{
			{ _("%3.0f"),   _("%3.0f"),   _("%3.0f"),   _("%3.0f") }, // Short display
			{ _("R:%3.0f"), _("G:%3.0f"), _("B:%3.0f"), _("A:%3.0f") }, // Long display for RGBA
			{ _("H:%3.0f"), _("S:%3.0f"), _("V:%3.0f"), _("A:%3.0f") }  // Long display for HSVA
		};
		const char* fmt_table_float[3][4] =
		{
			{ _("%0.3f"),   _("%0.3f"),   _("%0.3f"),   _("%0.3f") }, // Short display
			{ _("R:%0.3f"), _("G:%0.3f"), _("B:%0.3f"), _("A:%0.3f") }, // Long display for RGBA
			{ _("H:%0.3f"), _("S:%0.3f"), _("V:%0.3f"), _("A:%0.3f") }  // Long display for HSVA
		};
	...
	else if ((flags & ImGuiColorEditFlags_HEX) != 0 && (flags & ImGuiColorEditFlags_NoInputs) == 0)
	{
		...
		if (InputText(_("##Text"), buf, IM_ARRAYSIZE(buf), ImGuiInputTextFlags_CharsHexadecimal | ImGuiInputTextFlags_CharsUppercase))
		{
			value_changed = true;
			char* p = buf;
			...
			i[0] = i[1] = i[2] = i[3] = 0;
			if (alpha)
				sscanf(p, _("%02X%02X%02X%02X"), (unsigned int*)&i[0], (unsigned int*)&i[1], (unsigned int*)&i[2], (unsigned int*)&i[3]);
			else
				sscanf(p, _("%02X%02X%02X"), (unsigned int*)&i[0], (unsigned int*)&i[1], (unsigned int*)&i[2]);
		}
		if (!(flags & ImGuiColorEditFlags_NoOptions))
			OpenPopupOnItemClick(_("context"));
		PopItemWidth();
...
bool ImGui::ColorPicker4(const char* label, float col[4], ImGuiColorEditFlags flags, const float* ref_col)
{
	...
	if (!(flags & ImGuiColorEditFlags_NoSidePreview))
	{
		...
		if ((flags & ImGuiColorEditFlags_NoLabel))
			Text(_("Current"));
		ColorButton(_("##current"), col_v4, (flags & (ImGuiColorEditFlags_HDR | ImGuiColorEditFlags_AlphaPreview | ImGuiColorEditFlags_AlphaPreviewHalf | ImGuiColorEditFlags_NoTooltip)), ImVec2(square_sz * 3, square_sz * 2));
		if (ref_col != NULL)
		{
			Text(_("Original"));
			ImVec4 ref_col_v4(ref_col[0], ref_col[1], ref_col[2], (flags & ImGuiColorEditFlags_NoAlpha) ? 1.0f : ref_col[3]);
			if (ColorButton(_("##original"), ref_col_v4, (flags & (ImGuiColorEditFlags_HDR | ImGuiColorEditFlags_AlphaPreview | ImGuiColorEditFlags_AlphaPreviewHalf | ImGuiColorEditFlags_NoTooltip)), ImVec2(square_sz * 3, square_sz * 2)))
			{
	...
	if ((flags & ImGuiColorEditFlags_NoInputs) == 0)
	{
		...
		if (flags & ImGuiColorEditFlags_RGB || (flags & ImGuiColorEditFlags__InputsMask) == 0)
			value_changed |= ColorEdit4(_("##rgb"), col, sub_flags | ImGuiColorEditFlags_RGB);
		if (flags & ImGuiColorEditFlags_HSV || (flags & ImGuiColorEditFlags__InputsMask) == 0)
			value_changed |= ColorEdit4(_("##hsv"), col, sub_flags | ImGuiColorEditFlags_HSV);
		if (flags & ImGuiColorEditFlags_HEX || (flags & ImGuiColorEditFlags__InputsMask) == 0)
			value_changed |= ColorEdit4(_("##hex"), col, sub_flags | ImGuiColorEditFlags_HEX);
		PopItemWidth();
	}
...
bool ImGui::BeginDragDropSource(ImGuiDragDropFlags flags, int mouse_button)
{
	...
	{
		window = NULL;
		source_id = ImHash(_("#SourceExtern"), 0);
...
bool ImGui::SetDragDropPayload(const char* type, const void* data, size_t data_size, ImGuiCond cond)
{
	...
	IM_ASSERT(type != NULL);
	IM_ASSERT(strlen(type) < IM_ARRAYSIZE(payload.DataType) && _("Payload type can be at most 12 characters long"));
...
{
	if (ImGui::Begin(_("ImGui Metrics"), p_open))
	{
		ImGui::Text(_("Dear ImGui %s"), ImGui::GetVersion());
		ImGui::Text(_("Application average %.3f ms/frame (%.1f FPS)"), 1000.0f / ImGui::GetIO().Framerate, ImGui::GetIO().Framerate);
		ImGui::Text(_("%d vertices, %d indices (%d triangles)"), ImGui::GetIO().MetricsRenderVertices, ImGui::GetIO().MetricsRenderIndices, ImGui::GetIO().MetricsRenderIndices / 3);
		ImGui::Text(_("%d allocations"), ImGui::GetIO().MetricsAllocs);
		static bool show_clip_rects = true;
		ImGui::Checkbox(_("Show clipping rectangles when hovering an ImDrawCmd"), &show_clip_rects);
		ImGui::Separator();
		...
		struct Funcs
		{
			...
			{
				bool node_open = ImGui::TreeNode(draw_list, _("%s: '%s' %d vtx, %d indices, %d cmds"), label, draw_list->_OwnerName ? draw_list->_OwnerName : _(""), draw_list->VtxBuffer.Size, draw_list->IdxBuffer.Size, draw_list->CmdBuffer.Size);
				if (draw_list == ImGui::GetWindowDrawList())
				{
					ImGui::SameLine();
					ImGui::TextColored(ImColor(255, 100, 100), _("CURRENTLY APPENDING")); // Can't display stats for active draw list! (we don't have the data double-buffered)
				...
				for (const ImDrawCmd* pcmd = draw_list->CmdBuffer.begin(); pcmd < draw_list->CmdBuffer.end(); elem_offset += pcmd->ElemCount, pcmd++)
				{
					...
					{
						ImGui::BulletText(_("Callback %p, user_data %p"), pcmd->UserCallback, pcmd->UserCallbackData);
						continue;
					}
					ImDrawIdx* idx_buffer = (draw_list->IdxBuffer.Size > 0) ? draw_list->IdxBuffer.Data : NULL;
					bool pcmd_node_open = ImGui::TreeNode((void*)(pcmd - draw_list->CmdBuffer.begin()), _("Draw %-4d %s vtx, tex = %p, clip_rect = (%.0f,%.0f)..(%.0f,%.0f)"), pcmd->ElemCount, draw_list->IdxBuffer.Size > 0 ? _("indexed") : _("non-indexed"), pcmd->TextureId, pcmd->ClipRect.x, pcmd->ClipRect.y, pcmd->ClipRect.z, pcmd->ClipRect.w);
					...
					while (clipper.Step())
						for (int prim = clipper.DisplayStart, vtx_i = elem_offset + clipper.DisplayStart * 3; prim < clipper.DisplayEnd; prim++)
						{
							...
							for (int n = 0; n < 3; n++, vtx_i++)
							{
								...
								buf_p += ImFormatString(buf_p, (int)(buf_end - buf_p), _("%s %04d { pos = (%8.2f,%8.2f), uv = (%.6f,%.6f), col = %08X }\n"), (n == 0) ? _("vtx") : _("   "), vtx_i, v.pos.x, v.pos.y, v.uv.x, v.uv.y, v.col);
							}
			...
			{
				if (!ImGui::TreeNode(label, _("%s (%d)"), label, windows.Size))
					return;
				for (int i = 0; i < windows.Size; i++)
					Funcs::NodeWindow(windows[i], _("Window"));
				ImGui::TreePop();
			}

			static void NodeWindow(ImGuiWindow* window, const char* label)
			{
				if (!ImGui::TreeNode(window, _("%s '%s', %d @ 0x%p"), label, window->Name, window->Active || window->WasActive, window))
					return;
				NodeDrawList(window, window->DrawList, _("DrawList"));
				ImGui::BulletText(_("Pos: (%.1f,%.1f), Size: (%.1f,%.1f), SizeContents (%.1f,%.1f)"), window->Pos.x, window->Pos.y, window->Size.x, window->Size.y, window->SizeContents.x, window->SizeContents.y);
				ImGui::BulletText(_("Scroll: (%.2f/%.2f,%.2f/%.2f)"), window->Scroll.x, GetScrollMaxX(window), window->Scroll.y, GetScrollMaxY(window));
				ImGui::BulletText(_("Active: %d, WriteAccessed: %d"), window->Active, window->WriteAccessed);
				if (window->RootWindow != window) NodeWindow(window->RootWindow, _("RootWindow"));
				if (window->DC.ChildWindows.Size > 0) NodeWindows(window->DC.ChildWindows, _("ChildWindows"));
				ImGui::BulletText(_("Storage: %d bytes"), window->StateStorage.Data.Size * (int)sizeof(ImGuiStorage::Pair));
				ImGui::TreePop();
			}
		};
		...
		ImGuiContext& g = *GImGui;
		Funcs::NodeWindows(g.Windows, _("Windows"));
		if (ImGui::TreeNode(_("DrawList"), _("Active DrawLists (%d)"), g.DrawDataBuilder.Layers[0].Size))
		{
			for (int i = 0; i < g.DrawDataBuilder.Layers[0].Size; i++)
				Funcs::NodeDrawList(NULL, g.DrawDataBuilder.Layers[0][i], _("DrawList"));
			ImGui::TreePop();
		}
		if (ImGui::TreeNode(_("Popups"), _("Open Popups Stack (%d)"), g.OpenPopupStack.Size))
		{
			for (int i = 0; i < g.OpenPopupStack.Size; i++)
			{
				ImGuiWindow* window = g.OpenPopupStack[i].Window;
				ImGui::BulletText(_("PopupID: %08x, Window: '%s'%s%s"), g.OpenPopupStack[i].PopupId, window ? window->Name : _("NULL"), window && (window->Flags & ImGuiWindowFlags_ChildWindow) ? _(" ChildWindow") : _(""), window && (window->Flags & ImGuiWindowFlags_ChildMenu) ? _(" ChildMenu") : _(""));
			}
			ImGui::TreePop();
		}
		if (ImGui::TreeNode(_("Basic state")))
		{
			ImGui::Text(_("HoveredWindow: '%s'"), g.HoveredWindow ? g.HoveredWindow->Name : _("NULL"));
			ImGui::Text(_("HoveredRootWindow: '%s'"), g.HoveredRootWindow ? g.HoveredRootWindow->Name : _("NULL"));
			ImGui::Text(_("HoveredId: 0x%08X/0x%08X (%.2f sec)"), g.HoveredId, g.HoveredIdPreviousFrame, g.HoveredIdTimer);
			ImGui::Text(_("ActiveId: 0x%08X/0x%08X (%.2f sec)"), g.ActiveId, g.ActiveIdPreviousFrame, g.ActiveIdTimer);
			ImGui::Text(_("ActiveIdWindow: '%s'"), g.ActiveIdWindow ? g.ActiveIdWindow->Name : _("NULL"));
			ImGui::Text(_("NavWindow: '%s'"), g.NavWindow ? g.NavWindow->Name : _("NULL"));
			ImGui::Text(_("DragDrop: %d, SourceId = 0x%08X, Payload \"%s\" (%d bytes)"), g.DragDropActive, g.DragDropPayload.SourceId, g.DragDropPayload.DataType, g.DragDropPayload.DataSize);
			ImGui::TreePop();
		}
	}
	ImGui::End();
