...
#ifdef __clang__
#pragma clang diagnostic ignored _("-Wold-style-cast")         // warning : use of old-style cast                              // yes, they are more terse.
...
ImFont* ImFontAtlas::AddFontDefault(const ImFontConfig* font_cfg_template)
{
	...
	if (font_cfg.Name[0] == '\0') strcpy(font_cfg.Name, _("ProggyClean.ttf, 13px"));
...
{
	int data_size = 0;
	void* data = ImFileLoadToMemory(filename, _("rb"), &data_size, 0);
	if (!data)
	{
	...
	if (font_cfg.Name[0] == '\0')
	{
		...
		SNPRINTF(font_cfg.Name, IM_ARRAYSIZE(font_cfg.Name), _("%s, %.0fpx"), p, size_pixels);
	}
